    ...
    internal class BrotliCompressCommand : CommandBase
    {
        ...
        protected override Task<int> ExecuteCoreAsync()
        {
            Parallel.For(0, Sources.Values.Count, i =>
            {
                ...
                if (File.Exists(output) && File.GetLastWriteTimeUtc(source) < File.GetLastWriteTimeUtc(output))
                {
                    // Incrementalism. If input source doesn't exist or it exists and is not newer than the expected output, do nothing.
                    Out.WriteLine($"Skipping '{source}' because '{output}' is newer than '{source}'.");
                    return;
                }
